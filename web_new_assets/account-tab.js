...
(function () {
    ...
    function ensureOrdersSearchShell(container) {
        ...
        if (input && summary && list) {
            return { input: input, summary: summary, list: list };
        }
    ...
    function buildAccountShellMarkup() {
        return `
            <div class='fixed inset-0 z-[85] bg-black/50 modal-shell flex items-center justify-center p-4' id='account-settings-page' onclick='if(event.target === this) closeSettings()'>
            ...
            <div class='fixed inset-0 z-[90] bg-black/50 modal-shell flex items-center justify-center p-4' id='address-book-modal' onclick='if(event.target === this) closeAddressBookModal()'>
            ...
            <div class='fixed inset-0 z-[100] bg-black/50 modal-shell flex items-center justify-center p-4' id='address-form-modal' onclick='if(event.target === this) closeAddressFormModal()'>
            ...
            <div class='fixed inset-0 bg-black/50 z-[70] modal-shell flex items-center justify-center p-4' id='orders-page' onclick='if(event.target === this) closeOrders()'>
                <div class='modal-panel bg-gray-50 w-full max-w-4xl max-h-[90vh] rounded-[28px] shadow-2xl flex flex-col overflow-hidden' onclick='event.stopPropagation()'>
            ...
            <div class='fixed inset-0 bg-black/50 z-[70] modal-shell flex items-center justify-center p-4' id='wishlist-page' onclick='if(event.target === this) closeWishlist()'>
            ...
            <div id="settings-editor-panel" class="fixed inset-0 z-[120] bg-black/50 modal-shell flex items-center justify-center p-4 lg:hidden" onclick="if(event.target === this) closeProfileFieldEditor()">
    ...
    function ensureAccountFeatureShell() {
        ...
        if (!document.getElementById('account-settings-page') || !document.getElementById('orders-page') || !document.getElementById('wishlist-page') || !document.getElementById('address-book-modal')) {
    ...
    window.renderAccountTab = function () {
        ...
        const statusCards = [
            ...
            { label: 'Đã hủy', icon: 'fa-file-circle-xmark', color: 'text-red-500', count: orders.filter((order) => Number(order.rawStatus) === 5).length }
        ];
            ...
            <div class="lg:hidden w-full bg-gray-50 min-h-screen pb-24">
                ...
                <div class="bg-white mx-5 ${inactiveUser ? '' : '-mt-6'} p-0 rounded-[24px] shadow-sm border border-gray-100 relative z-10">
                    <div class="flex justify-between items-center px-4 py-3 border-b border-gray-50">
                        ...
                        <button onclick="openOrders()" class="text-xs font-bold text-gray-400 flex items-center gap-1 hover:text-babyPink">Lịch sử <i class="fa-solid fa-angle-right"></i></button>
                    </div>
                    <div class="flex justify-between items-center py-4 px-2">
                        ${statusCards.map(item => `
                            <button class="flex-1 flex flex-col items-center gap-2 relative" onclick="openOrders()">
        ...
        const pcLayout = `
            <div class='hidden lg:grid grid-cols-[340px,minmax(0,1fr)] gap-6 max-w-7xl mx-auto py-6'>
                <section class='bg-white rounded-[32px] border border-gray-100 shadow-sm p-6 flex flex-col items-start min-w-0 h-fit'>
                    ...
                    <button class='flex items-center justify-between w-full py-3 px-2 rounded-xl hover:bg-gray-50 group transition' onclick='openOrders()'>
                ...
                <div class='space-y-6 min-w-0'>
                    <section class='bg-white rounded-[32px] border border-gray-100 shadow-sm p-6'>
                        <div class='flex flex-wrap items-center justify-between gap-4 mb-5'>
                            ...
                            <button onclick='openOrders()' class="bg-gray-50 text-gray-600 px-4 py-2 rounded-xl text-xs font-bold hover:bg-gray-100 transition border border-gray-200">Xem tất cả</button>
                        </div>
                        <div class='grid grid-cols-4 gap-3'>
                            ${statusCards.map(item => `
                                <button class='bg-gray-50 rounded-2xl border border-gray-100 p-4 flex flex-col items-center justify-center gap-2 hover:border-pink-200 hover:bg-pink-50 transition group' onclick='openOrders()'>
    ...
    window.openSettings = function () {
        ...
        const shell = document.getElementById('account-settings-page');
    ...
    window.openAddressBookModal = function () {
        ...
        const shell = document.getElementById('address-book-modal');
        if (shell) shell.classList.add('is-open');
    };
    ...
    window.openAddressFormModal = function (addressId = null) {
        ...
        const shell = document.getElementById('address-form-modal');
        if (shell) shell.classList.add('is-open');
    };
    ...
    window.saveAllSettingsPC = async function () {
        ...
        if (fileInput && fileInput.files && fileInput.files[0]) {
            try {
                ...
                return bridge.showToast && bridge.showToast('Upload ảnh thất bại.', 'warning');
            }
        }
    ...
    window.renderWishlistUI = function () {
        ...
        const items = products.filter((product) => wishedIds.has(String((product && product.id) || '').trim()));
        container.innerHTML = items.map((product) => `
            <div class="bg-white rounded-2xl p-3 shadow-sm border border-gray-100 relative group flex flex-col h-full hover:border-pink-200 transition">
                ...
                <button onclick="toggleWishlist(event, '${product.id}')" class="absolute top-4 right-4 z-10 w-8 h-8 bg-white/90 backdrop-blur rounded-full flex items-center justify-center shadow-sm text-babyPink hover:scale-110 transition"><i class="fa-solid fa-heart"></i></button>
                <div onclick="openProductDetail('${product.id}')" class="cursor-pointer flex-1 flex flex-col">
                    <div class="rounded-xl overflow-hidden mb-3">
                        <img src="${bridge.getOptimizedImageUrl ? bridge.getOptimizedImageUrl(product.img, 'w600') : product.img}" loading="lazy" decoding="async" class="w-full h-32 md:h-48 object-cover group-hover:scale-105 transition duration-500" />
                    </div>
                    <h4 class="text-sm font-bold text-gray-800 line-clamp-2 leading-snug mb-2 group-hover:text-babyPink transition">${bridge.getProductDisplayName ? bridge.getProductDisplayName(product) : product.name}</h4>
                ...
                ${(order.items || []).map((item) => `
                    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-3 flex gap-3">
                        <div class="w-20 h-20 rounded-xl overflow-hidden shrink-0 border border-gray-50">
                            <img src="${bridge.getOptimizedImageUrl ? bridge.getOptimizedImageUrl(item.img, 'w400') : item.img}" loading="lazy" decoding="async" class="w-full h-full object-cover"/>
                        </div>
    ...
    window.renderOrdersUI = function () {
        ...
        const filteredOrders = filterOrdersByKeyword(sortedOrders);
        const searchSummary = orderSearchKeyword
            ? `<p class="mt-2 text-xs font-bold text-gray-400 text-center">Tìm thấy ${filteredOrders.length}/${orders.length} đơn hàng.</p>`
            : '';
        ...
        if (shell.list) {
            ...
            const statusTone = rawStatus === 5
                ...
                : (rawStatus === 4 ? 'text-emerald-600 bg-emerald-50 border-emerald-100' : 'text-orange-500 bg-orange-50 border-orange-100');
            return `
