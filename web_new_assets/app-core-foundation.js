...
// Đổi 1 lần tại đây để cập nhật số liên hệ, Zalo, kho và link hỗ trợ toàn web.
    ...
    const getExplicitWebpImageUrl = (raw, size = 'w800') => {
        ...
        const optimizedUrl = getOptimizedImageUrl(value, size);
        if(!/^https?:\/\/res\.cloudinary\.com\//i.test(optimizedUrl) || !optimizedUrl.includes('/image/upload/')) return '';
        if(optimizedUrl.includes('/image/upload/f_auto,q_auto,')) {
            return optimizedUrl.replace('/image/upload/f_auto,q_auto,', '/image/upload/f_webp,q_auto,');
        }
        if(optimizedUrl.includes('/image/upload/f_auto,q_auto/')) {
            return optimizedUrl.replace('/image/upload/f_auto,q_auto/', '/image/upload/f_webp,q_auto/');
        }
        return optimizedUrl.replace('/image/upload/', '/image/upload/f_webp,q_auto/');
    };
    const getOptimizedImageUrl = (raw, size = 'w800') => {
        ...
        if(/^https?:\/\/res\.cloudinary\.com\//i.test(value) && value.includes('/image/upload/')) {
            ...
            return finalizeOptimizedImageUrl(value.replace('/image/upload/', `/image/upload/${transforms.join(',')}/`));
        }
            ...
            <div class="order-success-invoice-shell">
                <div class="order-success-invoice-top">
                    ...
                    <div class="order-success-status-box">
                        ...
                        <strong>${payload.persisted === false ? 'Đã ghi nhận trên web' : 'Đã tạo trên hệ thống'}</strong>
                    </div>
                </div>
    ...
    const ensureOrderSuccessModalShell = () => {
        ...
        wrapper.innerHTML = `
            <div class="fixed inset-0 z-[96] bg-black/50 modal-shell flex items-center justify-center p-4" id="order-success-overlay" onclick="if(event.target === this) closeOrderSuccessModal()">
    ...
    window.openOrderSuccessModal = (payload = {}) => {
        ...
        if(title) {
            title.innerText = isUpdatedOrder ? 'Cập nhật đơn hàng thành công' : 'Đặt hàng thành công';
        }
        if(msg) {
            msg.innerText = isUpdatedOrder
